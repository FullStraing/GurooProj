...
# Логин пользователя (добавил обработку GETзапроса)
...
def user_logout(request):
    ...
    # Очищаем кэш, чтобы кнопка Наза не работала
...
def send_verification_code(request):
    if request.method == "POST":
        ...
        #  Добавляем SELECT только нужных данных
...
def complete_registration(request):
    if request.method == "POST":
        ...
        try:
            ...
            #  Если пользователь уже существует, просто входим в аккаунт
            print(f" Пользователь {email} уже существует, выполняем вход...")
...
def edit_tutor_profile(request):
    ...
    # Инициализация переменных для GETзапроса
    course = None
    subject = None
...
def send_tutor_request(request):
    if request.method == "POST":
        try:
            ...
            if not created:
                print(" Запись уже существует:", relation)
